...
WEB_APP_URL = os.getenv("WEB_APP_URL", "https://kdjebr.onrender.com/").rstrip("/") + "/"

# Owner always has access for free.
OWNER_ID = 8958072114
...
async def start(message: Message):
    ...
    # Owner gets a direct free entry.
    if user_id == OWNER_ID:
        await message.answer(
            "👑 <b>SAVE SNOSER</b>\n\n"
            "Ты владелец бота.\n"
            "Доступ к Mini App для тебя всегда бесплатный.",
...
async def select_plan(callback):
    ...
    await send_invoice(callback, callback.data)
...
async def successful_payment(message: Message):
    ...
    old_expiration = old_user[3] if old_user else 0

    if plan["duration"] == 0:
        expires_at = 0
    else:
        start_time = old_expiration if old_expiration and old_expiration > now else now
    ...
    access_text = (
        ...
        if plan["duration"] == 0
            ...
            "⏳ Доступ активен на <b>1 день</b>."
            if plan["duration"] == 86400
            else "⏳ Доступ активен на <b>7 дней</b>."
        )
    )
            ...
            return web.json_response(
                {
                    ...
                    "owner": True,
                    "expires_at": 0,
                    "expires_text": "Владелец — навсегда"
                }
            )
...
async def on_startup(app):
    ...
    if not render_url:
        # WEB_APP_URL is also a valid fallback for local/testing.
...
async def main():
    ...
    # IMPORTANT:
    # The Mini App is now served from the root URL.
